...
// import { roomId, myTurn } from './main.js'; // Removed to break circular dependency
...
export let board, turn, playerColor, lastMove, history;
let chess3D = null;
...
export function movePiece(from, to) {
  ...
  // Record move for history before modifying board
  const moveRecord = {
    from,
    to,
    piece: movingPiece,
    ...
    for (let f = 0; f < 8; f++) {
      ...
      cell.onclick = () => {
        ...
        if (!selected && board[pos] && board[pos].color === currentColor) {
          if (turn !== playerColor) return; // ❌ 非我方回合 → 不可选中
          selected = pos;
